...
const filePath = 'C:\\SHAT_Company\\shat-company-platform\\assets\\js\\translations.js';
...
// Now append/inject into translations.js
for (const lang of ['ar', 'en', 'fr', 'es', 'it']) {
  const navSearch = `${lang}: {`;
  const navIdx = content.indexOf(navSearch);
  if (navIdx !== -1) {
    // Add nav.academy
    const navTag = `nav: {`;
    const innerNavIdx = content.indexOf(navTag, navIdx);
    if (innerNavIdx !== -1) {
      const insertNavPos = innerNavIdx + navTag.length;
      content = content.slice(0, insertNavPos) + `\n      academy: "${academyData[lang].navAcademy}",` + content.slice(insertNavPos);
    }
    
    // Add academy object before footer in that language
    const footerTag = `footer: {`;
    const footerIdx = content.indexOf(footerTag, navIdx);
    if (footerIdx !== -1) {
      const academyStr = `\n    academy: ${JSON.stringify(academyData[lang], null, 6)},\n`;
      content = content.slice(0, footerIdx) + academyStr + content.slice(footerIdx);
    }
  }
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully updated translations.js with Academy data for all 5 languages!');
